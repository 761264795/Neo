...
type
  TFMLotin = class(TI3BaseFrm)
    ...
    cdsOrgListFID: TWideStringField;
    cdsOrgListFnumber: TWideStringField;
    cdsOrgListFname_l2: TWideStringField;
    Label1: TLabel;
    cdsOrgListCFBRanchFlag: TWideStringField;
...
begin
  ...
  LoginStates := False;
  if not CliDM.Get_OpenSQL(cdsOrgList,'select fid,fnumber,fname_l2,CFBRanchFlag from t_org_baseunit where FID<>''11111111-1111-1111-1111-111111111111CCE7AED4''  order by Flevel,fnumber',ErrMsg) then
  begin
...
begin
  ...
  try
    ...
    try
      if not Ini_ShopInfo then  //下载服务器注册信息
      begin
        ShowMsg(Handle,'下载初始化店铺信息报错！',[]);
        abort; //初始化参数信息
      end;
    except
      on e : Exception do
      begin
        ShowMsg(Handle,'下载初始化店铺信息报错:'+E.Message,[]);
        abort; //初始化参数信息
      end;
    end;
  finally
...
var ReturnUserFID,FPERSONID,ErrorMsg : string;
begin
  Result := True;
  ...
  try
    ...
    lbl1.Visible:=true;
    application.ProcessMessages;
    if Chk_LoginUser(UserInfo.Is_SysOnline, UserInfo.ClientTag, Trim(edtUserID.Text), Trim(edtUserPwd.Text), CliDM.cdsUserShopList,CliDM.cdsShopIntervalList, ReturnUserFID,ErrMsg,FPERSONID) = 0 then
    begin
...
begin
  try
    ...
    _SQL :=' select a.FID,a.fnumber,a.fName_l2,a.CFBRanchFlag,a.FControlUnitID from t_org_baseunit a '
