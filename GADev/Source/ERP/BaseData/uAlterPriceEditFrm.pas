...
uses
  ...
  cxDBLookupComboBox, cxCurrencyEdit, cxCheckBox, cxButtonEdit;

type
  TAlterPriceEditFrm = class(TBasedataEditFrm)
    Label3: TLabel;
    ...
    Label7: TLabel;
    txt_FSaleOrgUnitName: TcxDBButtonEdit;
    cdsMaterFSaleOrgUnitID: TWideStringField;
    cdsSaveFSaleOrgUnitID: TWideStringField;
    cdsMaterFSaleOrgUnitName: TStringField;
    ...
    procedure txt_FSaleOrgUnitNamePropertiesButtonClick(Sender: TObject;
...
var
  AlterPriceEditFrm: TAlterPriceEditFrm;

implementation
 uses FrmCliDM,Pub_Fun,uMaterDataSelectHelper,uDrpHelperClase,uPriceAlterGuide;
{$R *.dfm}

{ TAlterPriceEditFrm }

function TAlterPriceEditFrm.CHK_Data: Boolean;
begin
  result := true;
  if txt_FSaleOrgUnitName.Text = '' then
  begin
    ShowMsg(self.Handle,'销售组织不能为空！   ',[]);
    txt_FSaleOrgUnitName.SetFocus;
    Result := False;
    Exit;
  end;
end;

function TAlterPriceEditFrm.chk_Repeat: Boolean;
begin

end;

function TAlterPriceEditFrm.CopyNewAdd: Boolean;
...
begin
  ...
  try
    ...
    try
      ...
      _SQL := 'insert into T_I3_PRICEPOLICYMaterialENTRY '
             +' (FID,FParentID,FSEQ,FCustomerID,FMaterialID,FPriceTypeID,FDiscount,FPrice,FDESCRIPTION) '
             +' select SYS_GUID(),'+Quotedstr(BillFID)+',FSEQ,FCustomerID,FMaterialID,FPriceTypeID,FDiscount,FPrice,FDESCRIPTION from '
             +' T_I3_PRICEPOLICYMaterialENTRY where FParentID='+Quotedstr(OldFID);
...
function TAlterPriceEditFrm.DelData: Boolean;
...
function TAlterPriceEditFrm.OpenData(mFID: String): Boolean;
...
begin
  try
    ...
    begin
      ...
      _SQL[0] := materSQL;
      _SQL[1] := 'select * from T_I3_PRICEPOLICY '+' where fid='+Quotedstr(mFID);
      _SQL[2] := 'select a.*,b.fnumber as FCustomerNumber,b.fname_l2 as FCustomerName from T_I3_PRICEPOLICYBranchENTRY a '
               + ' left join t_bd_customer b  on a.fcustomerid = b.fid'+' where a.fparentid='+Quotedstr(mFID)+' order by a.FSEQ';
      _SQL[3] := 'select * from T_I3_PRICEPOLICYBranchENTRY where fparentid='+Quotedstr(mFID);
    end
    else
    begin
      MaterStatus := 0;
      ...
      _SQL[0] := materSQL;
      _SQL[1] := 'select * from T_I3_PRICEPOLICY '+' where 1<>1';
      _SQL[2] := 'select a.*,b.fnumber as FCustomerNumber,b.fname_l2 as FCustomerName from T_I3_PRICEPOLICYBranchENTRY a '
               + ' left join t_bd_customer b  on a.fcustomerid = b.fid'+' where 1<>1';
      _SQL[3] := 'select * from T_I3_PRICEPOLICYBranchENTRY where 1<>1';
    end;
    if not (CliDM.Get_OpenClients_E(mFID,_cds,_SQL,ErrMsg)) then
    begin
      showmsg(self.Handle,'渠道调价单打开出错:'+ErrMsg,[]);
      Self.Close;
      Abort;
    end;
    if mFID = '' then
...
function TAlterPriceEditFrm.ST_Save: Boolean;
...
begin
  try
    ...
    try
      if CliDM.Apply_Delta_E(_cds,['T_I3_PRICEPOLICY','T_I3_PRICEPOLICYBranchENTRY'],error) then
      begin
        Result := True;
        ...
        Gio.AddShow('T_I3_PRICEPOLICY表提交成功！');
      end
      else
      begin
      ...
      on E: Exception do
      begin
        Gio.AddShow('T_I3_PRICEPOLICY表提交失败！'+e.Message);
...
procedure TAlterPriceEditFrm.FormCreate(Sender: TObject);
begin
  ...
  TcxLookupComboBoxProperties(cxGridMaterialListFPRICETYPEID.Properties).ListSource := CliDM.dsPriceType;
end;

procedure TAlterPriceEditFrm.cdsBranchEntryNewRecord(DataSet: TDataSet);
begin
  inherited;
...
procedure TAlterPriceEditFrm.cdsMaterialEntryNewRecord(DataSet: TDataSet);
begin
  inherited;
  DataSet.FieldByName('FID').AsString := Get_GuID;
  DataSet.FieldByName('Fparentid').AsString := cdsMater.fieldbyname('FID').AsString;
...
procedure TAlterPriceEditFrm.OpenMaterialList;
var materSQL,ErrMsg:string;
...
begin
  ...
  try
    ...
    _SQL[0] := 'select a.*,m.fnumber as FMaterialNumber,m.fname_l2 as FMaterialName,m.cfdistributeprice as FDisPrice'
             ...
             +' order by a.FSEQ';
    _SQL[1] := 'select * from T_I3_PRICEPOLICYMaterialENTRY '
    ...
    begin
      showmsg(self.Handle,'渠道调价单明细打开出错:'+ErrMsg,[]);
      Self.Close;
      Abort;
    end;
  finally
...
procedure TAlterPriceEditFrm.btn_ColorNewRowClick(Sender: TObject);
begin
  inherited;
  if MaterStatus <> 0 then
  begin
  ...
  try
    cxDetail.OnFocusedRecordChanged := nil;
    cdsBranchEntry.DisableControls;
    with Select_Customer(GetSelectedFIDs(cdsBranchEntry,'FCustomerID'),'','',0) do
    begin
      if Not IsEmpty then
      begin
        while not eof do
...
procedure TAlterPriceEditFrm.btn_ColorDelRowClick(Sender: TObject);
begin
  inherited;
  if MaterStatus <> 0 then
  begin
...
procedure TAlterPriceEditFrm.SpeedButton2Click(Sender: TObject);
var _SQL:string;
begin
  inherited;   
  if MaterStatus <> 0 then
  ...
  begin
    showmsg(self.Handle,'请先选择客户或选中客户再新增物料',[]);
    Abort;
  end;
  _SQL :=' select a.fid,a.fnumber,a.fname_l2,b.cfdistributeprice from t_bd_material a left join t_bd_materialsales b '
        +' on a.fid=b.fmaterialid and b.forgunit= '+quotedstr(cdsMater.fieldbyname('FSaleOrgUnitID').AsString)
        +' where a.fstatus=1';
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            cdsMaterialEntry.FieldByName('FDisPrice').AsFloat := FieldByName('cfdistributeprice').AsFloat;
...
procedure TAlterPriceEditFrm.SpeedButton3Click(Sender: TObject);
begin
  inherited;
  if MaterStatus <> 0 then
  begin
...
function TAlterPriceEditFrm.GetStPrice(mFID:string): double;
...
procedure TAlterPriceEditFrm.cdsMaterialEntryFPRICEValidate(
  Sender: TField);
begin
  inherited;
  try
...
procedure TAlterPriceEditFrm.cdsMaterialEntryFDISCOUNTValidate(
  Sender: TField);
begin
  inherited;
  try
...
procedure TAlterPriceEditFrm.cdsBranchEntryBeforePost(DataSet: TDataSet);
var i:integer;
begin
  inherited;
  if  cdsSaveBranchEntry.Locate('FID',vararrayof([cdsBranchEntry.fieldbyname('fid').AsString]),[]) then
...
procedure TAlterPriceEditFrm.cdsMaterialEntryBeforePost(DataSet: TDataSet);
var i:integer;
begin
  inherited;
  if  cdsSaveMaterialEntry.Locate('FID',vararrayof([cdsMaterialEntry.fieldbyname('fid').AsString]),[]) then
...
procedure TAlterPriceEditFrm.cdsMaterNewRecord(DataSet: TDataSet);
begin
  inherited;
  Dataset.FieldByName('Fnumber').AsString := 'QDTJ-'+formatdatetime('yyyymmddhhhmmsss',now);
  Dataset.FieldByName('FBranchID').AsString := userinfo.Branch_ID;
end;

function TAlterPriceEditFrm.SaveMaterialList: boolean;
...
procedure TAlterPriceEditFrm.cxDetailFocusedRecordChanged(
...
procedure TAlterPriceEditFrm.cxDetailEditing(
...
procedure TAlterPriceEditFrm.cxGridMaterialListEditing(
  Sender: TcxCustomGridTableView; AItem: TcxCustomGridTableItem;
  var AAllow: Boolean);
begin
  inherited;
...
procedure TAlterPriceEditFrm.cdsBranchEntryAfterOpen(DataSet: TDataSet);
begin
  inherited;
  sptAdjust.Enabled := Dataset.IsEmpty;
end;

procedure TAlterPriceEditFrm.sptAdjustClick(Sender: TObject);
var sFID : string;
begin
  inherited;
  if not ST_Save then exit;
  sFID := cdsmater.fieldbyname('FID').AsString;
  if CallPriceAlterGuide(cdsmater.fieldbyname('FID').AsString,cdsmater.fieldbyname('FSaleOrgUnitID').AsString) then
  begin
    OpenData(sFID);
  end;
end;

procedure TAlterPriceEditFrm.cdsMaterFSTATUSGetText(Sender: TField;
...
procedure TAlterPriceEditFrm.spt_DetailDownCopyClick(Sender: TObject);
var bk:string;
    tmp:Variant;
begin
  inherited;
...
procedure TAlterPriceEditFrm.spt_MaterialDownCopyClick(Sender: TObject);
var bk:string;
    tmp:Variant;
begin
  inherited;
...
procedure TAlterPriceEditFrm.txt_FSaleOrgUnitNamePropertiesButtonClick(
...
begin
  ...
  begin
    ...
    begin
      cdsMater.Edit;
      cdsMater.FieldByName('FSaleOrgUnitID').AsString := fieldbyname('FID').AsString;
    end;
  end;
end;

procedure TAlterPriceEditFrm.cdsMaterCalcFields(DataSet: TDataSet);
var SaleOrgID,_SQL,ErrMsg:string;
begin
  inherited;
  SaleOrgID := Trim(cdsMater.FieldByName('FSaleOrgUnitID').AsString);
  if SaleOrgID <> '' then
  begin
    _SQL := 'select Fname_l2 from t_org_baseunit where fid='+quotedstr(SaleOrgID);
    DataSet.FieldByName('FSaleOrgUnitName').AsString := VarToStr(CliDM.Get_QueryReturn(_SQL,ErrMsg));
  end;
end;
