inherited ServiceAttributeAllocationFrm: TServiceAttributeAllocationFrm
  Left = 210
  Top = 108
  Width = 954
  Height = 610
  Caption = #19994#21153#23646#24615#20998#37197
  OldCreateOrder = True
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 12
  object Panel1: TPanel [0]
    Left = 0
    Top = 0
    Width = 946
    Height = 38
    Align = alTop
    Color = 16511980
    TabOrder = 0
    object Label1: TLabel
      Left = 8
      Top = 7
      Width = 193
      Height = 25
      AutoSize = False
      Caption = #19994#21153#23646#24615#20998#37197
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -21
      Font.Name = #23435#20307
      Font.Style = []
      ParentFont = False
    end
  end
  object Panel2: TPanel [1]
    Left = 0
    Top = 38
    Width = 946
    Height = 503
    Align = alClient
    BevelOuter = bvNone
    Color = 16511980
    TabOrder = 1
    object Splitter1: TSplitter
      Left = 597
      Top = 0
      Height = 503
      Align = alRight
    end
    object Panel3: TPanel
      Left = 600
      Top = 0
      Width = 346
      Height = 503
      Align = alRight
      BevelOuter = bvNone
      Color = 16511980
      TabOrder = 0
      object Panel4: TPanel
        Left = 0
        Top = 0
        Width = 346
        Height = 26
        Align = alTop
        Color = 16511980
        TabOrder = 0
        DesignSize = (
          346
          26)
        object btn_Add: TSpeedButton
          Left = 194
          Top = 1
          Width = 75
          Height = 24
          Anchors = [akTop, akRight]
          Caption = #26032#22686#34892
          Flat = True
          Glyph.Data = {
            
            FF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00}
          OnClick = btn_AddClick
        end
        object btn_Del: TSpeedButton
          Left = 269
          Top = 1
          Width = 75
          Height = 24
          Anchors = [akTop, akRight]
          Caption = #21024#38500#34892
          Flat = True
          Glyph.Data = {
            
            FF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00}
          OnClick = btn_DelClick
        end
        object Label4: TLabel
          Left = 6
          Top = 7
          Width = 77
          Height = 12
          AutoSize = False
          Caption = #21487#20998#37197#26426#26500
        end
      end
      object cxGrid2: TcxGrid
        Left = 0
        Top = 26
        Width = 346
        Height = 477
        Align = alClient
        TabOrder = 1
        LookAndFeel.Kind = lfOffice11
        object cxGridBranch: TcxGridDBTableView
          NavigatorButtons.ConfirmDelete = False
          DataController.DataSource = dsBranch
          DataController.Summary.DefaultGroupSummaryItems = <>
          DataController.Summary.FooterSummaryItems = <
            item
              Kind = skCount
              FieldName = 'FBranchNumber'
              Column = cxGridBranchFBranchNumber
            end>
          DataController.Summary.SummaryGroups = <>
          OptionsData.CancelOnExit = False
          OptionsData.Deleting = False
          OptionsData.DeletingConfirmation = False
          OptionsData.Editing = False
          OptionsData.Inserting = False
          OptionsView.DataRowHeight = 22
          OptionsView.Footer = True
          OptionsView.GroupByBox = False
          OptionsView.HeaderHeight = 22
          object cxGridBranchFID: TcxGridDBColumn
            DataBinding.FieldName = 'FID'
            Visible = False
          end
          object cxGridBranchFBranchNumber: TcxGridDBColumn
            DataBinding.FieldName = 'FBranchNumber'
            Width = 110
          end
          object cxGridBranchFBranchName: TcxGridDBColumn
            DataBinding.FieldName = 'FBranchName'
            Width = 229
          end
        end
        object cxGridLevel1: TcxGridLevel
          GridView = cxGridBranch
        end
      end
    end
    object Panel5: TPanel
      Left = 0
      Top = 0
      Width = 597
      Height = 503
      Align = alClient
      BevelOuter = bvNone
      Color = 16511980
      TabOrder = 1
      object cxGrid1: TcxGrid
        Left = 0
        Top = 56
        Width = 597
        Height = 447
        Align = alClient
        TabOrder = 0
        LookAndFeel.Kind = lfOffice11
        object cxGridMaterial: TcxGridDBTableView
          NavigatorButtons.ConfirmDelete = False
          DataController.DataSource = dsMaterial
          DataController.Summary.DefaultGroupSummaryItems = <>
          DataController.Summary.FooterSummaryItems = <
            item
              Kind = skCount
              FieldName = 'SELECTED'
              Column = cxGridMaterialSELECTED
            end>
          DataController.Summary.SummaryGroups = <>
          OptionsData.CancelOnExit = False
          OptionsData.Deleting = False
          OptionsData.DeletingConfirmation = False
          OptionsData.Inserting = False
          OptionsView.DataRowHeight = 22
          OptionsView.Footer = True
          OptionsView.GroupByBox = False
          OptionsView.HeaderHeight = 22
          object cxGridMaterialSELECTED: TcxGridDBColumn
            DataBinding.FieldName = 'SELECTED'
            PropertiesClassName = 'TcxCheckBoxProperties'
            Properties.NullStyle = nssUnchecked
            Properties.ValueChecked = 1
            Properties.ValueUnchecked = 0
            Width = 40
          end
          object cxGridMaterialFID: TcxGridDBColumn
            Caption = 'a'
            DataBinding.FieldName = 'FID'
            Visible = False
            Options.Editing = False
            Width = 80
          end
          object cxGridMaterialFNUMBER: TcxGridDBColumn
            DataBinding.FieldName = 'FNUMBER'
            Options.Editing = False
            Width = 100
          end
          object cxGridMaterialFNAME_L2: TcxGridDBColumn
            DataBinding.FieldName = 'FNAME_L2'
            Options.Editing = False
            Width = 125
          end
          object cxGridMaterialBRANDNAME: TcxGridDBColumn
            DataBinding.FieldName = 'BRANDNAME'
            Options.Editing = False
            Width = 80
          end
          object cxGridMaterialYEARSNAME: TcxGridDBColumn
            DataBinding.FieldName = 'YEARSNAME'
            Options.Editing = False
            Width = 80
          end
          object cxGridMaterialSEASONNAME: TcxGridDBColumn
            DataBinding.FieldName = 'SEASONNAME'
            Options.Editing = False
            Width = 80
          end
          object cxGridMaterialATTRNAME: TcxGridDBColumn
            DataBinding.FieldName = 'ATTRNAME'
            Options.Editing = False
            Width = 80
          end
        end
        object cxGrid1Level1: TcxGridLevel
          GridView = cxGridMaterial
        end
      end
      object Panel7: TPanel
        Left = 0
        Top = 0
        Width = 597
        Height = 56
        Align = alTop
        Color = 16511980
        TabOrder = 1
        DesignSize = (
          597
          56)
        object Label3: TLabel
          Left = 5
          Top = 38
          Width = 52
          Height = 12
          AutoSize = False
          Caption = #29289#26009#36807#28388
        end
        object Label5: TLabel
          Left = 5
          Top = 9
          Width = 52
          Height = 12
          AutoSize = False
          Caption = #19994#21153#23646#24615
        end
        object Label6: TLabel
          Left = 253
          Top = 9
          Width = 52
          Height = 12
          AutoSize = False
          Caption = #32452#32455#21333#20803
        end
        object Bevel1: TBevel
          Left = -1
          Top = 28
          Width = 591
          Height = 2
          Anchors = [akLeft, akTop, akRight]
        end
        object txtMaterial: TcxTextEdit
          Left = 61
          Top = 33
          Properties.OnChange = txtMaterialPropertiesChange
          TabOrder = 0
          Width = 182
        end
        object btn_AllSelect: TcxButton
          Left = 495
          Top = 32
          Width = 44
          Height = 23
          Anchors = [akTop, akRight]
          Caption = #20840#36873
          TabOrder = 1
          OnClick = btn_AllSelectClick
          LookAndFeel.Kind = lfOffice11
        end
        object btn_NotSelect: TcxButton
          Left = 541
          Top = 32
          Width = 44
          Height = 23
          Anchors = [akTop, akRight]
          Caption = #21453#36873
          TabOrder = 2
          OnClick = btn_NotSelectClick
          LookAndFeel.Kind = lfOffice11
        end
        object txt_Org: TcxButtonEdit
          Left = 307
          Top = 4
          Properties.Buttons = <
            item
              Default = True
              Kind = bkEllipsis
            end>
          Properties.OnButtonClick = txt_OrgPropertiesButtonClick
          TabOrder = 3
          OnKeyPress = txt_OrgKeyPress
          Width = 182
        end
        object cb_OrgType: TcxComboBox
          Left = 61
          Top = 4
          Properties.DropDownListStyle = lsFixedList
          Properties.Items.Strings = (
            #36130#21153#36164#26009
            #38144#21806#36164#26009
            #37319#36141#36164#26009
            #24211#23384#36164#26009
            #25104#26412#36164#26009)
          TabOrder = 4
          Text = #36130#21153#36164#26009
          Width = 183
        end
        object btn_Find: TcxButton
          Left = 503
          Top = 3
          Width = 68
          Height = 23
          Anchors = [akTop, akRight]
          Caption = #26597#25214'(&F)'
          TabOrder = 5
          OnClick = btn_FindClick
          LookAndFeel.Kind = lfOffice11
        end
      end
    end
  end
  object Panel8: TPanel [2]
    Left = 0
    Top = 541
    Width = 946
    Height = 38
    Align = alBottom
    Color = 16511980
    TabOrder = 2
    DesignSize = (
      946
      38)
    object Label2: TLabel
      Left = 5
      Top = 12
      Width = 388
      Height = 12
      AutoSize = False
      Caption = #25552#31034#65306#20197#19978#29289#26009#20026#24403#21069#26426#26500#21487#20998#37197#21040#20854#23427#26426#26500#30340#29289#26009#12290
    end
    object btn_Alter: TcxButton
      Left = 755
      Top = 7
      Width = 85
      Height = 25
      Anchors = [akTop, akRight]
      Caption = #24320#22987#20998#37197'(&S)'
      TabOrder = 0
      OnClick = btn_AlterClick
      LookAndFeel.Kind = lfOffice11
    end
    object btn_Exit: TcxButton
      Left = 847
      Top = 7
      Width = 75
      Height = 25
      Anchors = [akTop, akRight]
      Caption = #36864#20986'(&E)'
      TabOrder = 1
      OnClick = btn_ExitClick
      LookAndFeel.Kind = lfOffice11
    end
  end
  inherited STBaseEditDataSet: TClientDataSet
    Left = 40
    Top = 432
  end
  object cdsMaterial: TClientDataSet
    Aggregates = <>
    Params = <>
    OnFilterRecord = cdsMaterialFilterRecord
    Left = 96
    Top = 214
    object cdsMaterialSELECTED: TFloatField
      DisplayLabel = #36873#25321
      FieldName = 'SELECTED'
    end
    object cdsMaterialFID: TWideStringField
      FieldName = 'FID'
      Size = 44
    end
    object cdsMaterialFNUMBER: TWideStringField
      DisplayLabel = #29289#26009#32534#21495
      FieldName = 'FNUMBER'
      Size = 80
    end
    object cdsMaterialFNAME_L2: TWideStringField
      DisplayLabel = #29289#26009#21517#31216
      FieldName = 'FNAME_L2'
      Size = 510
    end
    object cdsMaterialBRANDNAME: TWideStringField
      DisplayLabel = #21697#29260
      FieldName = 'BRANDNAME'
      Size = 510
    end
    object cdsMaterialYEARSNAME: TWideStringField
      DisplayLabel = #24180#20221
      FieldName = 'YEARSNAME'
      Size = 510
    end
    object cdsMaterialATTRNAME: TWideStringField
      DisplayLabel = #27874#27573
      FieldName = 'ATTRNAME'
      Size = 510
    end
    object cdsMaterialSEASONNAME: TWideStringField
      DisplayLabel = #23395#33410
      FieldName = 'SEASONNAME'
      Size = 510
    end
  end
  object dsMaterial: TDataSource
    DataSet = cdsMaterial
    Left = 176
    Top = 214
  end
  object cdsBranch: TClientDataSet
    Aggregates = <>
    Params = <>
    Left = 696
    Top = 374
    object cdsBranchFID: TStringField
      FieldName = 'FID'
      Size = 60
    end
    object cdsBranchFBranchNumber: TStringField
      DisplayLabel = #26426#26500#32534#21495
      FieldName = 'FBranchNumber'
      Size = 80
    end
    object cdsBranchFBranchName: TStringField
      DisplayLabel = #26426#26500#21517#31216
      FieldName = 'FBranchName'
      Size = 80
    end
  end
  object dsBranch: TDataSource
    DataSet = cdsBranch
    Left = 784
    Top = 374
  end
  object cdsCompany: TClientDataSet
    Aggregates = <>
    Params = <>
    Left = 385
    Top = 98
  end
  object cdsInventory: TClientDataSet
    Aggregates = <>
    Params = <>
    Left = 385
    Top = 170
  end
  object cdsPur: TClientDataSet
    Aggregates = <>
    Params = <>
    Left = 386
    Top = 306
  end
  object cdsCost: TClientDataSet
    Aggregates = <>
    Params = <>
    Left = 385
    Top = 242
  end
  object cdsSale: TClientDataSet
    Aggregates = <>
    Params = <>
    Left = 386
    Top = 362
  end
  object cdsCompany_save: TClientDataSet
    Aggregates = <>
    Params = <>
    Left = 481
    Top = 98
  end
  object cdsInventory_save: TClientDataSet
    Aggregates = <>
    Params = <>
    OnNewRecord = cdsInventory_saveNewRecord
    Left = 481
    Top = 170
  end
  object cdsCost_save: TClientDataSet
    Aggregates = <>
    Params = <>
    Left = 481
    Top = 242
  end
  object cdsPur_save: TClientDataSet
    Aggregates = <>
    Params = <>
    Left = 482
    Top = 306
  end
  object cdsSale_save: TClientDataSet
    Aggregates = <>
    Params = <>
    Left = 482
    Top = 362
  end
end
