...
type
  TAlterPriceQuery = class(TSTBaseEdit)
    Panel1: TPanel;
    cBegin: TcxDateEdit;
    cxLabel1: TcxLabel;
    cEnd: TcxDateEdit;
...
var
  AlterPriceQuery: TAlterPriceQuery;
  function GetAlterPriceQuerySQL(var _SQL:string):boolean;
implementation
  uses FrmCliDM,Pub_Fun,uMaterDataSelectHelper,uDrpHelperClase,StringUtilClass,DateUtils;
{$R *.dfm}
function GeTAlterPriceQuerySQL(var _SQL:string):boolean;
begin
  application.CreateForm(TAlterPriceQuery,AlterPriceQuery);
  result :=  AlterPriceQuery.ShowModal = mrOK;
  _SQL := AlterPriceQuery.WhereStr;
  AlterPriceQuery.Free;
end;
procedure TAlterPriceQuery.txt_FBranchNameKeyPress(Sender: TObject;
  var Key: Char);
begin
  inherited;
  if Key <> #8 then  Key := #0
...
procedure TAlterPriceQuery.txt_ctNameKeyPress(Sender: TObject;
  var Key: Char);
begin
  inherited;
  if Key <> #8 then  Key := #0
...
procedure TAlterPriceQuery.txt_auNameKeyPress(Sender: TObject;
  var Key: Char);
begin
  inherited;
  if Key <> #8 then  Key := #0
...
procedure TAlterPriceQuery.btn1Click(Sender: TObject);
begin
  inherited;
...
procedure TAlterPriceQuery.cxButton1Click(Sender: TObject);
begin
  inherited;
  close;
end;

procedure TAlterPriceQuery.FormShow(Sender: TObject);
begin
  inherited;
  cbegin.Date  := Date;
  aBegin.Date  := Date;
...
procedure TAlterPriceQuery.txt_FBranchNamePropertiesButtonClick(
...
procedure TAlterPriceQuery.txt_ctNamePropertiesButtonClick(
  Sender: TObject; AButtonIndex: Integer);
begin
  inherited;
...
procedure TAlterPriceQuery.txt_auNamePropertiesButtonClick(
  Sender: TObject; AButtonIndex: Integer);
begin
  inherited;
