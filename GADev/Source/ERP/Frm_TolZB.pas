...
uses
  ...
  cxCurrencyEdit, Menus, ActnList,ADODB,DateUtils,
  Graphics, cxClasses, cxControls, StrUtils, cxStyles, cxGraphics,
  cxFilter, cxData, cxDataStorage, cxEdit, jpeg,cxGridExportLink;
...
uses uExportExcelFrm,materialinfo,FrmCliDM;
...
begin
 {
  try
    ...
    begin
      ...
      +quotedstr(UpperCase(G_UserInfo.UserID))+' and  Report_ID=''' + trim(Report_ID) + ''' order by lsh'
      else
...
begin
  ...
  callExcelExport(nil,cgdList,FReport_name);
...
begin
  ...
  try
    ...
    cgdList.BeginUpdate;
    FilePath:=ExtractFilePath(Application.ExeName)+userinfo.LoginUser+'\ZBFX\';
...
procedure TFM_TolZB.BitBtn1Click(Sender: TObject);
begin
  new_ZBFX;
  CliDM.ClientUserLog(Self.Caption,'占比统计','来源窗口:'+Self.Caption);
end;
...
begin
  if TADODataSet(cgdList.DataController.DataSource.DataSet).FindField('material_id')=nil then
  begin
    showmessage(pchar('报表中未找到[material_id]字段!     '));
    Abort;
  end;
  if not TADODataSet(cgdList.DataController.DataSource.DataSet).IsEmpty then
  getMaterialinfo(TADODataSet(cgdList.DataController.DataSource.DataSet).fieldbyname('material_id').AsString);
end;

procedure TFM_TolZB.SpeedButton5Click(Sender: TObject);
begin
  if cgdList.DataController.DataSource.DataSet.IsEmpty then Exit;
  if SaveDg.Execute then
  ExportGridToExcel(SaveDg.FileName, cxGrid1, True, true, True);
end;
