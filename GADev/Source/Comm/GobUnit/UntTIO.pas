...
var
  ...
  G_IsAlterListView:boolean=false;
  G_Log:boolean;
implementation
...
begin
  Result := 0;
  try
    EnterCriticalSection(X_thread);     //进入临界区
    try
    ...
    LeaveCriticalSection(X_thread);  //离开临界区
  end;
end;
...
var
  ...
  Is_ClearLog: Boolean;
begin
  if G_IsAlterListView then exit;
  if (LogLevel=0)  then
  begin
    if (not G_Log) then    exit;
  end;
  Is_ClearLog := False;
  Result := -1;
