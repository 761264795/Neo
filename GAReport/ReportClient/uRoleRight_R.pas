...
type
  TRoleRightFrm = class(TForm)
    ...
    procedure csdUserCalcFields(DataSet: TDataSet);
...
var
  RoleRightFrm: TRoleRightFrm;

implementation
  uses uMpsPub_R,ActiveX,uReportPub_R;
...
procedure TRoleRightFrm.FormShow(Sender: TObject);
begin
  try
...
procedure TRoleRightFrm.dxbrlrgbtnExitEditClick(Sender: TObject);
begin
  Close;
end;

procedure TRoleRightFrm.csdUserAfterScroll(DataSet: TDataSet);
begin
  if not DataSet.IsEmpty  then
  begin
    stat1.Panels[0].Text:='您正在给【'+Dataset.fieldbyname('FNAME_L2').AsString+'】角色授权...';
    Self.Caption:='角色权限管理->【'+Dataset.fieldbyname('FNAME_L2').AsString+'】';
  end;
end;



procedure TRoleRightFrm.csdUserCalcFields(DataSet: TDataSet);
begin
  //csdUserimg.AsInteger:=1;
end;

procedure TRoleRightFrm.dxbrlrgbtnSaveBillClick(Sender: TObject);
begin
...
procedure TRoleRightFrm.cdsSeeNewRecord(DataSet: TDataSet);
begin
  DataSet.FieldByName('ID').AsString := getGuid ;
  DataSet.FieldByName('user_id').AsString :=csdUser.fieldbyname('FID').AsString;
end;

procedure TRoleRightFrm.cdsModuleNewRecord(DataSet: TDataSet);
begin
  DataSet.FieldByName('ID').AsString := getGuid ;
end;

procedure TRoleRightFrm.cdsModuleListNewRecord(DataSet: TDataSet);
begin
  DataSet.FieldByName('ID').AsString := getGuid ;
end;

procedure TRoleRightFrm.N1Click(Sender: TObject);
...
procedure TRoleRightFrm.cxGridModuleListEnter(Sender: TObject);
begin
  grid:=2;
end;

procedure TRoleRightFrm.N2Click(Sender: TObject);
var bookmark: TBookmarkStr;
begin
  if (grid=2) then
  begin
...
procedure TRoleRightFrm.N4Click(Sender: TObject);
begin
  close;
end;

procedure TRoleRightFrm.cxGriduserDBTvCustomDrawCell(
...
procedure TRoleRightFrm.cdsSeeReconcileError(DataSet: TCustomClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure TRoleRightFrm.cdsModuleReconcileError(DataSet: TCustomClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
  showmsg('数据保存失败!原因：' + e.Message);
...
procedure TRoleRightFrm.cdsModuleListReconcileError(
...
procedure TRoleRightFrm.dxBarLargeButton1Click(Sender: TObject);
begin
...
procedure TRoleRightFrm.openData(user_id: string);
begin
  if csdUser.IsEmpty then Exit;
  with cdsSee do
  begin
    Close;
    CommandText:='select * from ROLEFIELDPDM   where upper(user_id) ='''+uppercase(trim(user_id))+'''';
    Open;
  end;
  cdsModule.Filtered:=False;
  with cdsModule do
  begin
    try
      ...
      Close;
      CommandText:='select * from F_ROLEMODULE   where upper(user_id) ='''+uppercase(trim(user_id))+''' order by id';
      Open;
    finally
      cxGridModuleDBTv.OnFocusedRecordChanged:=cxGridModuleDBTvFocusedRecordChanged;
...
procedure TRoleRightFrm.openListData(user_id: string);
begin
  with cdsModuleList do
  begin
    Close;
    CommandText:='select * from F_ROLEMODULE_LIST   where upper(user_id) ='''+uppercase(trim(user_id))+''' and MODULE_ID='''+trim(cdsModule.fieldbyname('MODULE_ID').AsString)+''' order by id';
    Open;
  end;
end;

procedure TRoleRightFrm.cxGriduserDBTvFocusedRecordChanged(
  Sender: TcxCustomGridTableView; APrevFocusedRecord,
  AFocusedRecord: TcxCustomGridRecord;
  ANewItemRecordFocusingChanged: Boolean);
begin
...
procedure TRoleRightFrm.cxGridModuleDBTvFocusedRecordChanged(
  Sender: TcxCustomGridTableView; APrevFocusedRecord,
  AFocusedRecord: TcxCustomGridRecord;
  ANewItemRecordFocusingChanged: Boolean);
begin
...
procedure TRoleRightFrm.MenuItem1Click(Sender: TObject);
var bookmark: TBookmarkStr;
begin
...
procedure TRoleRightFrm.MenuItem2Click(Sender: TObject);
var bookmark: TBookmarkStr;
begin
  if cdsModule.RecordCount > 0 then
  begin
...
procedure TRoleRightFrm.Label1Click(Sender: TObject);
begin
  cdsSee.Edit;
...
procedure TRoleRightFrm.Label2Click(Sender: TObject);
begin
  cdsSee.Edit;
...
procedure TRoleRightFrm.Label3Click(Sender: TObject);
begin
  cdsSee.Edit;
...
procedure TRoleRightFrm.Label4Click(Sender: TObject);
begin
  cdsSee.Edit;
...
procedure TRoleRightFrm.EaeSysCreateData;
begin
  try
    ...
    begin
      ...
      Close;    //增加报表项目
      CommandText:=' insert into F_ROLEMODULE(id,Isselect,user_id,Module_Id,MODULE_NAME)'
         +' select  sys_guid() as id ,0 as Isselect, c.user_id,c.Module_Id,rp.reporttypename as MODULE_NAME from '
         +' (select a.FID as user_id,b.report_id as Module_Id from t_Pm_Role a ,F_report b   '
         +' where not exists(select 1 from F_ROLEMODULE m where m.MODULE_ID=b.report_id and m.user_id=a.FID) '
         +' order by  a.fnumber ) c left join REPORTTYPE rp on c. Module_Id=rp.treeid ' ;
      Execute;
      Close;    //增加报表子项目
      CommandText:='insert into F_ROLEMODULE_List(id,Isselect,Module_Id,MODULE_list_ID,MODULE_list_NAME,user_id)  '
              +' select  sys_guid() as id ,1 as Isselect, c.Module_Id,c.fmd_id,c.fmd_name ,c.user_id from'
              +' (select a.user_id as user_id,a.Module_Id as Module_Id,b.fmd_id ,b.fmd_name from F_ROLEMODULE a ,F_MODELTITLE_RP b'
              +' where not exists(select 1 from F_ROLEMODULE_List m where m.MODULE_ID=a.Module_Id and m.user_id=a.user_id ) and b.ftype=2 and upper(a.Module_Id)<>''MAINFRM'' '
    ...
    on e:exception do
    begin
      Gio_R.AddShow(e.Message+':'+DMPub.db_update.CommandText);
    end;
  end;
end;

procedure TRoleRightFrm.cxGriduserEnter(Sender: TObject);
begin
  G_Dataset:=csdUser;
  FindTitle:='角色定位';
end;

procedure TRoleRightFrm.cxGridModuleEnter(Sender: TObject);
begin
  G_Dataset:=cdsModule;
  FindTitle:='报表定位';
end;

procedure TRoleRightFrm.FindBillListClick(Sender: TObject);
var bk:TBookmarkStr;
    b:Boolean;
    val:string;
begin
...
procedure TRoleRightFrm.ReportTreeCustomDrawItem(Sender: TCustomTreeView;
  Node: TTreeNode; State: TCustomDrawState; var DefaultDraw: Boolean);
begin
...
procedure TRoleRightFrm.ReportTreeFocusedNodeChanged(
...
procedure TRoleRightFrm.cdsModuleFilterRecord(DataSet: TDataSet;
  var Accept: Boolean);
  var treeid:string;
begin
...
procedure TRoleRightFrm.cdsTypeListFilterRecord(DataSet: TDataSet;
  var Accept: Boolean);
