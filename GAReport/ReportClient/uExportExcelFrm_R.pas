...
type
  ...
  public
    ...
    procedure exportExcel_Ex;
  end;

var
  ExportExcelFrm_R: TExportExcelFrm;
  procedure callExcelExport(PehGrid:TdbgridEH;PcxGrid:TcxGridDBTableView;rtNM:string;isEx:boolean);
implementation
   uses STPublic_R,DMPublic_R,uReportPub_R;
{$R *.dfm}
procedure callExcelExport(PehGrid:TdbgridEH;PcxGrid:TcxGridDBTableView;rtNM:string;isEx:boolean);
begin
  try
    ...
    Application.CreateForm(TExportExcelFrm,ExportExcelFrm_R);
    ExportExcelFrm_R._ehGrid:=PehGrid;
    ExportExcelFrm_R._cxGrid:=PcxGrid;
    ExportExcelFrm_R._isEx:=isEx;
    ExportExcelFrm_R._reportName:=rtNM;
    ExportExcelFrm_R.ShowModal;
  finally
    saveLog(DateTimeToStr(Now)+'结束导出excel...');
    ExportExcelFrm_R.Free;
  end;
end;
...
procedure TExportExcelFrm.exportExcel_Ex;
var
  col, row,e_row,maxCol,maxRow,memLines,e_page: Integer;
  sline: String;
  mem: TMemo;
...
begin
  ...
  try
    ...
    begin
      sline := '';
      //列格式设置
      FieldInfoVal := VarArrayCreate([0, fieldList.Count-1], varVariant);
      for col := 0 to fieldList.Count-1 do
      begin
        sline := sline + TrimX(dataset.fieldbyname(fieldList[col]).AsString) + #9;
        //列格式设置
        if dataset.fieldbyname(fieldList[col]).DataType in [ftString, ftMemo, ftFmtMemo, ftWideString] then
          FieldInfoVal[col] := VarArrayOf([col+1, 2])
        else
          FieldInfoVal[col] := VarArrayOf([col+1, 1]);
      end;
      Inc(row);
      mem.Lines.Add(sline);
      maxRow:=maxRow+1;
    ...
    FilePath:=FilePath+GetGuid+'.txt';
    mem.Lines.SaveToFile(FilePath);
    Label1.Caption:='正在打开Excel...';
    Application.ProcessMessages;
    ExcelApp.Workbooks.OpenText(Filename:=FilePath, FieldInfo := FieldInfoVal);
    SetSizeCaptionToExcel(self._reportName,ExcelApp,maxCol);
    Application.ProcessMessages;
    Clipboard.Clear;
  finally
    dataset.GotoBookmark(bk);
