if exists (select 1 from sysobjects where name=UPPER('p_rpt_RepairWoDetail_TC') and xtype='P')
	drop procedure p_rpt_RepairWoDetail_TC;
go
create procedure p_rpt_RepairWoDetail_TC
...
select r.FNumber 维修工单号,u.FName_L2 制单人,p.FName_L2 服务顾问
      ...
      ,case when tp.CFRelateItemEntryId is not null then tp.CFRelateItemEntryId
           else re.CFRelateItemEntryId end 关联项目
  ...
  left join (select distinct CFRelateItemEntryId from CT_ATS_RepairWORWOItemSpEntry) tp
        on re.fid=tp.CFRelateItemEntryId
